...
# taken from https://github.com/tam-carre/polywins
...
inactive_right="%{F-}"
minimized_left="%{F$minimized_text_color}"
minimized_right="%{F-}"
...
generate_window_list() {
	...
	while IFS="[ .\.]" read -r wid ws cname cls host title; do
		...
		######## rules to filter out unwanted title parts
		title=$(echo ${title/tino/})								# hostname appears in front of overwatch
		title=$(echo ${title/exe /})								# exe appears after the name of wine processes
		title=$(echo ${title/ org.pwmt.zathura/})		# zathura pdf viewer has weird titles
		title=$(echo ${title/battle.net.exe /})			# another wine process which behaves weird
		title=$(echo ${title/Mozilla /})						# always appears  in front of firefox
		title=$(echo ${title/Org.gnome./})

		# format window titles further (only show program name)
		IFS='[—-|]' read -ra programNames <<< "$title"		# split title by '—' or '-'
		w_name=${programNames[-1]}		#show only last part
		w_name=`echo $w_name | sed 's/ *$//g'`			# remove leading spaces
		...
		windowState=$(xprop -id $wid | grep "_NET_WM_STATE(ATOM)")
		hiddenState="_NET_WM_STATE(ATOM) = _NET_WM_STATE_HIDDEN"
		# echo ">>>>>>>>>>>>>>>>>>>>>>>>>$windowState<<<<<<<<<<<<<<<<<<<<<<<<"

		# Add left and right formatting to displayed name
		if [ "$wid" = "$active_wid" ]; then
			w_name="${active_left}${w_name}${active_right}"
		elif [ "$windowState" = "$hiddenState" ]; then
			w_name="${minimized_left}${w_name}${minimized_right}"
		else
